...
El módulo BCD_4displays recibe una entrada binaria bcd_in de N_in bits que representa un número decimal completo y genera tres salidas (D_un, D_de, D_ce) correspondientes a unidades, decenas y centenas, respectivamente. El número de entrada se separa según su valor posicional utilizando operaciones aritméticas, y cada dígito resultante se convierte a su representación en display de 7 segmentos mediante la instanciación del mósulo BCD_module.
...
El módulo counter_100 implementa un contador ascendente–descendente limitado por el valor del parámetro CMAX. El contador opera con el flanco positivo del reloj (clk) e incluye una señal de reinicio (rst) que inicializa la salida. Mediante la señal updown se selecciona el sentido del conteo: ascendente cuando updown = 1 y descendente cuando updown = 0. Además, el módulo permite cargar un valor externo (data_in) directamente en la salida cuando la señal load está activa; de lo contrario, el contador continúa su operación normal. El conteo se reinicia al alcanzar los límites definidos por CMAX y cero, según el modo de operación.
